...
//        }
        ...
        let router = GenreRouter(endpoint: .getGenres)
        TmdbApiRouter.shared.request(cls: ResponseGenre.self, router: router) { arrGenres, error in
            if let arrGenres = arrGenres, error == nil {
                completion(arrGenres.genres)
            }
        }
    }
}
